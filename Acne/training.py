...
model.train(
    ...
    lr0=0.00003,                  # Initial learning rate
    lrf=0.005,                    # Final learning rate factor (final lr becomes lr0 * lrf)
    momentum=0.95,
    autoanchor=True,
    freeze=20,
    weight_decay=5e-5,           # Weight decay to mitigate overfitting
    mosaic=True,                # Disable mosaic augmentation for focused learning
    mixup=True,                 # Disable mixup to avoid distortion on a small dataset
    hsv_s=0.2,                   # Limited HSV saturation augmentation to reduce color variance
    hsv_v=0.3,                   # Limited HSV value augmentation to reduce color variance
    translate=0.05,              # Small translation augmentation
    scale=0.9,                   # Minor scaling augmentation
    patience=100,                 # Early stopping patience to avoid overfitting
